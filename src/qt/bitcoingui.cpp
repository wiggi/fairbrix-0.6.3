...
{
    resize(850, 550);
    setWindowTitle(tr("Fairbrix Wallet"));
#ifndef Q_WS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Fairbrix address"));
    ...
    toggleHideAction = new QAction(QIcon(":/icons/bitcoin"), tr("Show/Hide &Fairbrix"), this);
    toggleHideAction->setToolTip(tr("Show or hide the Fairbrix window"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Fairbrix client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
    ...
    {
        // Click on system tray icon triggers "show/hide Fairbrix"
...
void BitcoinGUI::setMining(bool mining, int hashrate)
{
    if (mining)
    {
        ...
        labelMiningIcon->setToolTip(tr("Mining Fairbrix at %1 hashes per second").arg(hashrate));
    }
    else
    {
        labelMiningIcon->setPixmap(QIcon(":/icons/mining_inactive").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
        labelMiningIcon->setToolTip(tr("Not mining Fairbrix"));
    }
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Fairbrix address or malformed URI parameters."));
    }
